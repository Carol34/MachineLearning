...
def pca(X = np.array([]), no_dims = 50):
    ...
    X -= X.mean(axis=0).reshape((1,D))
    #step2: find top no_dims eigenvectors of covariance matrix
    cov = np.dot(X.T, X)
    _, M = np.linalg.eig(cov)
    M = M[:, :no_dims]
    M = M / np.linalg.norm(M,axis=0).reshape((1,no_dims))

    #step3: construct the dataset XV
    Y = X.dot(M)
    print(Y.shape)
...
def reconstruction_error(orig = np.array([]), decompressed = np.array([])):
    ...
    """TODO: write your code here"""
    error = ((orig - decompressed)**2).sum()/orig.shape[0]   
    return error
