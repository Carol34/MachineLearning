...
def predict_labels(k, ytrain, dists):
	...
	"""
#####################################################
	knn_ind = np.argpartition(dists, k, axis=1)
	knn_label = np.zeros((dists.shape[0],k))
	ypred = np.zeros(1000,)
	for i in range(dists.shape[0]):
	    for j in range(k):
	        knn_label[i][j] = ytrain[knn_ind[i][j]]
	for i in range(knn_label.shape[0]):
	    unique,count = np.unique(knn_label[i],return_counts=True)
	    ypred[i] = unique[np.argmax(count)]
...
def compute_accuracy(y, ypred):
   ...
   #####################################################
	#				 YOUR CODE HERE					                    #
	acc = 1 - ((np.count_nonzero(y-ypred))/y.shape[0])
	#####################################################
	return acc
...
def find_best_k(K, ytrain, dists, yval):
   ...
   #####################################################
	#				 YOUR CODE HERE					                    #
	best_k = 0
	validation_accuracy = np.zeros((len(K),1))
	best_acc = 0
	i = 0
	for k in K:
