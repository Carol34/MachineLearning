...
def multinomial_predict(X, w, b):
    ...
    TODO: add your code here
    """   
    prob = np.dot(w,X.transpose())+b.reshape((C,1))
...
#        yi[ind == False] = 0
        yi = np.where(y == i,1,0)
        wi, bi = binary_train(X, yi, step_size=step_size, max_iterations=max_iterations)
        w[i] = np.copy(wi)
        b[i] = np.copy(bi) 
    
#    print("shape of trained w:",w.shape)
...
def OVR_predict(X, w, b):
    ...
    TODO: add your code here
    """
#    print("X=",X.shape,'N=', N,"D=","w=",w.shape,"b=",b.shape)
    prob = np.transpose(np.dot(X,w.transpose()))+b.reshape(C,1)
#    prob = prob - np.max(prob, axis = 0)
#    prob2 = 1/(1 + np.exp(-1*(prob)))
    preds = np.argmax(prob, axis = 0)
#    print("prob=",prob.shape,"prob2=",prob2.shape,"preds=",preds.shape,)
