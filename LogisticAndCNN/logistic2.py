...
def binary_train(X, y, w0=None, b0=None, step_size=0.5, max_iterations=1000):
    ...
    for j in range(max_iterations):
        W = np.transpose(np.repeat([w],N,axis = 0))
        B = np.repeat(b,N,axis = 0)
        der_W = (1 - 1/(1 + np.exp(-1*Y*((np.transpose(np.sum((np.transpose(X)*W),axis=0)))+B).reshape(N,1))))*-1*Y*X
        der_B = (1 - 1/(1 + np.exp(-1*Y*((np.transpose(np.sum((np.transpose(X)*W),axis=0)))+B).reshape(N,1))))*-1*Y
        w = w - step_size * np.mean(der_W, axis = 0)
        b = b - step_size * np.mean(der_B)
