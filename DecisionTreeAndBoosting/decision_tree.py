...
class TreeNode(object):
	...
	def split(self):
		def conditional_entropy(branches: List[List[int]]) -> float:
			...
			totals = np.sum(branch,axis=0)       
			p_Y = branch / totals            
			entropy = -(np.array([[i * np.log(i) if i > 0 else 0 for i in x]for x in p_Y])).sum(axis=0)            
			p_A = branch.sum(axis=0) / branch.sum(axis=0).sum()

			entropy = p_A.dot(entropy)
			return entropy
