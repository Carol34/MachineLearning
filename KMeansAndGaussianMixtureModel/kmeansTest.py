...
def transform_image(image, code_vectors):
    ...
    # treat each pixel of an image as a point xi, 
    # then perform K-means algorithm to cluster these points, 
    # and finally replace each pixel with its centroid.
    H, W, D = image.shape
    v = image.reshape((H * W, D))
    n_clu = code_vectors.shape[0]   
    cen = code_vectors
    dist = np.full((H * W,n_clu), np.inf)
    for k in range(n_clu):
        dist[:,k] = np.linalg.norm(v - cen[k,:],axis = 1) ** 2
